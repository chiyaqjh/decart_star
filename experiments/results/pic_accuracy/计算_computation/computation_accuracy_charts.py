...
def metric_for_scheme(data, metric, model_key=None):
    ...
    if model_key is not None:
        ...
        if metric == 'check_ms':
            ...
            return _mean_ms(block.get('query_times'))
        if metric == 'keygen_ms':
            # keygen is not persisted in result JSONs; use encrypt phase as setup-time proxy.
            return _mean_ms(block.get('encrypt_times'))
        if metric == 'total_ms':
            return _mean_ms(block.get('encrypt_times')) + _mean_ms(block.get('query_times')) + _mean_ms(block.get('decrypt_times'))
        return _mean_ms(block.get(metric))
    ...
    for _, block in models.items():
        if metric == 'check_ms':
            if block.get('check_times'):
                ...
                values.append(_mean_ms(block.get('query_times')))
        elif metric == 'keygen_ms':
            values.append(_mean_ms(block.get('encrypt_times')))
        elif metric == 'total_ms':
            values.append(_mean_ms(block.get('encrypt_times')) + _mean_ms(block.get('query_times')) + _mean_ms(block.get('decrypt_times')))
        else:
            values.append(_mean_ms(block.get(metric)))
...
def chart_2_keygen_vs_n(run_missing):
    ...
    # Keep system parameters fixed and use record_dim as a keygen-scale proxy.
    ...
    plot_multi_line(
        ...
        title='Key Generation Time vs n (5 Schemes, real data proxy)',
        x_scale='log',
    )
...
def chart_4_encrypt_vs_records(run_missing):
    ...
    plot_multi_line(
        ...
        y_label='Running time (ms)',
        title='Encrypt Time vs Data Records (policy size = 32)',
        x_scale='log',
    )
    save_figure(fig, '04_encrypt_vs_records_policy32.png')
...
def chart_5_check_vs_model_size(run_missing):
    ...
    for label in models:
        ...
        for scheme in SCHEMES:
            ...
            values_map[scheme].append(metric_for_scheme(data_map[scheme], metric='check_ms', model_key=model_key))
    ...
    plot_grouped_bar(
        ...
        x_label='Model size',
        y_label='Running time (ms)',
        title='Check Time vs Model Size (5 Schemes, real data proxy)',
    )
    save_figure(fig, '05_check_vs_model_size.png')


def chart_6_query_vs_records(run_missing):
    records = [10, 100, 500, 1000, 5000, 10000]

    def model_curve(model_label):
        model_key = MODEL_KEY_MAP[model_label]
    ...
    mlp100 = model_curve('MLP100')
    mlp10000 = model_curve('MLP10000')
    nn100000 = model_curve('NN100000')
    resnet = model_curve('ResNet')

    fig, axes = plt.subplots(2, 2, figsize=(15.2, 11.0), sharex=False, sharey=True)
    plot_multi_line(
        axes[0, 0],
        records,
        mlp100,
    ...
    plot_multi_line(
        ...
        x_label='Number of data records',
        y_label='Running time (ms)',
        title='Query Time (ResNet)',
        x_scale='log',
    )
    save_figure(fig, '06_query_vs_records_mlp100_mlp10000_nn100000_resnet.png')
...
def chart_7_decrypt_vs_result_size(run_missing):
    ...
    plot_multi_line(
        ...
        y_label='Running time (ms)',
        title='Decrypt Time vs Query Result Size (5 Schemes)',
        x_scale='log',
    )
    save_figure(fig, '07_decrypt_vs_result_size.png')
