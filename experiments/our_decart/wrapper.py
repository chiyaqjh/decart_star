# decart/experiments/our_decart/wrapper.py
"""
DeCart 方案实验包装器
用于对比实验的标准化接口
"""
...
class DeCartExperimentWrapper:
    ...
    def __init__(self, N: int = 64, n: int = 16):
        ...
        参数:
            ...
            n: 每块用户数
        """
        self.params = DeCartParams(N=N, n=n)
        self.curator = None
        self.db_server = None
        self.owners = {}
        self.queriers = {}
    ...
    def setup(self) -> float:
        ...
        self.curator = KeyCurator(scheme="decart", params=self.params)
        self.curator.setup()
    ...
    def encrypt_dataset(self, 
        ...
        try:
            ...
            # 方法2: 如果 pickle 失败，估算大小
            # 每个密文对象估算为 1MB（根据 TenSEAL 文档）
            estimated_size = 0
            if 'c6_i' in C_m:
                # 每个加密数据记录估算
                estimated_size += len(C_m['c6_i']) * 1024 * 1024  # 1MB per record
            if 'c1_i' in C_m:
                estimated_size += len(C_m['c1_i']) * 1024  # 1KB per policy component
            size = estimated_size
    ...
    def get_metrics(self) -> Dict:
        ...
        if metrics['communication_sizes']:
            ...
            metrics['avg_comm_size'] = np.mean(sizes)
            metrics['total_comm'] = sum(sizes)
