...
class OfflineSchemeExperimentWrapper:
    ...
    def __init__(self, N: int = 64, n: int = 16):
        ...
        # User key pairs (offline pre-distribution)
        self.user_keys = {}  # user_id -> {'pk': public_key, 'sk': secret_key}
        
        # Authorization relationships (established offline)
        ...
        self.metrics = {
            ...
            'query_times': [],
            'decrypt_times': [],
            'check_times': [],  # Specifically track Check algorithm time
            'communication_sizes': [],
            'memory_sizes': []
        }
    ...
    def setup(self) -> float:
        ...
        # Offline scheme only needs homomorphic-encryption initialization
        elapsed = time.perf_counter() - start
        self.metrics['setup_time'] = elapsed
        print(f"   Offline scheme initialization completed: {elapsed:.4f}s")
        return elapsed
    ...
    def register_user(self, user_id: int) -> Tuple[int, Any]:
        ...
        # Generate user key pair (simulate offline distribution)
        # In practice, each user should have different keys; this baseline uses simplified "virtual" keys
    ...
    def setup_authorization(self, owner_id: int, querier_id: int):
        ...
        self.authorizations[(owner_id, querier_id)] = True
        print(f"    Authorization established: user {querier_id} can access owner {owner_id}'s data")
    ...
    def encrypt_dataset(self, 
        ...
        # Encrypt each data record (using owner key)
        encrypted_data = []
        ...
        # Establish authorization for each querier in policy
        ...
        print(f"    Offline-scheme encryption: {elapsed*1000:.2f} ms, ciphertext size: {size/1024:.2f} KB")
    ...
    def execute_query(self,
        ...
        # Check authorization (completed offline, near-zero overhead)
        ...
        # Decrypt results (using querier key)
        ...
        print(f"      Offline-scheme query: {query_time*1000:.2f} ms")
        print(f"      Check time: 0.00 ms (completed offline)")
