...
class ExperimentConfig:
    ...
    def __post_init__(self):
        ...
        self.results_dir = resolve_results_dir(self.dataset, "experiments/results/data_new/scheme1_ccs23", "scheme1_ccs23", self.results_dir)
...
class ExperimentRunner:
    ...
    def generate_model(self, model_type: str, run_id: int = 0) -> Any:
        ...
        if model_type == 'dot':
            ...
            # Decision-tree model (used as a simplified baseline in CCS23)
    ...
    def run_single_experiment(self, run_id: int, model_type: str) -> Optional[Dict]:
        print(f"\n{'='*60}")
        print(f" [CCS23] Running experiment {run_id+1}/{self.config.num_runs} - model: {model_type}")
        print(f"{'='*60}")
        ...
        try:
            ...
            # "Encrypt" dataset (plaintext storage in this baseline)
            print(f"\n Storing data...")
            C_m, sk_h_s, ds_id = wrapper.encrypt_dataset(owner_id, data, policy)
            
            # Store dataset (no extra operation needed in CCS23)
            ...
            for query_index in range(query_repetitions):
                ...
                print(f"   {query_label} CCS23 query start, querier={active_querier_id}")
                ...
                if current_results is not None:
                    ...
                    print(f"   {query_label} CCS23 query done, results={len(current_results)}")
                else:
                    print(f"   {query_label} CCS23 query done, no results returned")
    ...
    def save_results(self):
        ...
        print(f"\n CCS23 experiment results saved:")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CCS23 scheme experiment")
    ...
    print("\n" + "="*80)
    print(" Launching CCS23 experiment (performance upper-bound baseline)")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("  CCS23 experiment completed")
    print("="*80)