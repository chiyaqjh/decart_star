# decart/experiments/naive_ccs23/runner.py
"""Naive CCS-2023 baseline experiment runner."""
...
class ExperimentRunner:
    ...
    def run_single_experiment(self, run_id: int, model_type: str) -> Optional[Dict]:
        print(f"\n{'=' * 60}")
        print(f" [Naive CCS-2023] 运行实验 {run_id + 1}/{self.config.num_runs} - 模型: {model_type}")
        print(f"{'=' * 60}")
        try:
            wrapper = NaiveCCS23ExperimentWrapper(N=self.config.N, n=self.config.n)
    ...
    def _compute_statistics(self):
        ...
        for model_type, model_data in self.results['models'].items():
            ...
            for field, summary_name in [
                ('setup_times', 'avg_setup_time'),
                ('keygen_times', 'avg_keygen_time'),
                ('register_times', 'avg_register_time'),
                ('check_times', 'avg_check_time'),
                ('encrypt_times', 'avg_encrypt_time'),
                ('query_times', 'avg_query_time'),
                ('decrypt_times', 'avg_decrypt_time'),
            ]:
                values = model_data[field]
                if values:
                    stats[summary_name] = float(np.mean(values))
                    stats[summary_name.replace('avg_', 'std_')] = float(np.std(values))
    ...
    def save_results(self):
        ...
        json_path = os.path.join(self.config.results_dir, f"naive_ccs23_exp_{timestamp}.json")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Naive CCS-2023 方案实验')
