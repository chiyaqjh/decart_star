...
class NaiveCCS23ExperimentWrapper(ServerSchemeExperimentWrapper):
    ...
    def encrypt_dataset(
        ...
        encrypted_data = []
        for record in data:
            encrypted_data.append(self.he.encrypt(record))

        envelope_key = os.urandom(32)
        serialized_payload = self._serialize_encrypted_package(encrypted_data, policy, metadata)
        wrapped_payload = self._encrypt_payload(envelope_key, serialized_payload)
        ...
        header = {
            ...
            'encapsulations': {},
            'policy': tuple(sorted(set(policy))),
            'ciphertext_commitment': hashlib.sha256(serialized_payload).digest(),
            'wrapped_payload_digest': hashlib.sha256(wrapped_payload).digest(),
        ...
        upload_size = len(wrapped_payload) + self._safe_obj_size(header, fallback=max(1, len(header['encapsulations'])) * 64)
    ...
    def execute_query(self, querier_id: int, owner_id: int, dataset_id: str, model: Any) -> Optional[List[float]]:
        ...
        retrieval_size = len(wrapped_payload) + self._safe_obj_size(header, fallback=256)
        req_payload = {
            'querier_id': querier_id,
            'owner_id': owner_id,
            'dataset_id': dataset_id,
            'query_token': query_token,
            'header_digest': header['wrapped_payload_digest'],
            'retrieved_ciphertext_size': retrieval_size,
            'encrypted_model': model,
            'model_type': model.get('type', 'dot_product') if isinstance(model, dict) else 'dot_product',
        }
        req_size = self._safe_obj_size(req_payload) + retrieval_size
        self.metrics['communication_sizes'].append({
            'type': 'query',
            'size': req_size,
        ...
        response_size = self._safe_obj_size(results, fallback=max(1, len(results)) * 1024)
        self.metrics['communication_sizes'].append({
            'type': 'decrypt',
            'size': response_size,
            'records': len(results),
