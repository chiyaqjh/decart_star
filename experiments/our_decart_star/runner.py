...
class ExperimentRunner:
    ...
    def run_single_experiment(self, run_id: int, model_type: str) -> Optional[Dict]:
        """运行单次实验（指定模型类型）"""
        print(f"\n{'='*60}")
        print(f" [DeCart*] 运行实验 {run_id+1}/{self.config.num_runs} - 模型: {model_type}")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-types", nargs="+", 
                       ...
                       help="模型类型列表")
    parser.add_argument("--results-dir", type=str, default="experiments/results/our_decart_star", help="结果保存目录")
